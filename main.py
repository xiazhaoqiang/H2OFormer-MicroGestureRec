...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/smg_ssi.yaml',
        help='path to the configuration file')

    parser.add_argument(
        '--decoder',
        default='linear',
        help='Whether need the decoder -> None, `interpolate`, `linear`, `conv`')
    parser.add_argument(
        '--loss-rec',
        default='sum_mse',
        help='The loss function. eg. `sum_mse`, `mean_mse`, `batch_mse`, `sce`, `sum_sce`')
    parser.add_argument(
        '--weighted-loss',
        default='',
        help='Whether the loss is weighted, eg. `dynamic`, `weighted-0.9` means the class loss weighted 0.9')

    # mask
    parser.add_argument(
        '--mask-mode',
        default=None,
        help='The mode of masking points, eg. None, `random`, `group`')
    parser.add_argument(
        '--mask-id',
        default=[3],
        help='The [index] of the groups which is masked, available only when mask mode is `group`, eg. [3] or [3, 4]')
    parser.add_argument(
        '--mask-ratio',
        default=0.30,
        help='The ratio of the masked points in all points')

    parser.add_argument('-model_saved_name', default='')

    # processor
    parser.add_argument(
        '--phase', default='train', help='must be train or val')
    parser.add_argument(
        '--save-score',
        type=str2bool,
        default=False,
    ...
    parser.add_argument(
        ...
        action=DictAction,
        default=dict(),
        help='the arguments of data loader for val')
    parser.add_argument(
        '--test-feeder-args',
        action=DictAction,
        default=dict(),
    ...
    # model
    parser.add_argument('--model', default="model.Hyperformer.Model", help='the model will be used')
    parser.add_argument(
        '--model-args',
        action=DictAction,
        default=dict(),
    ...
    parser.add_argument(
        ...
        default=[0, 1],
        nargs='+',
        help='the indexes of GPUs for training or validating')
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if arg.phase == 'train':
            if not arg.train_feeder_args['debug']:
                ...
                self.train_writer = self.val_writer = SummaryWriter(os.path.join(arg.model_saved_name, 'val'), 'val')
        self.global_step = 0
        # pdb.set_trace()
        self.load_model()
        ...
        self.best_video_acc = 0
        self.best_video_acc_epoch = 0
    ...
    def print_log(self, str, print_time=True):
        ...
        if self.arg.print_log:
            with open('{}/a_log.txt'.format(self.arg.work_dir), 'a') as f:
                print(str, file=f)
    ...
    def train(self, epoch, save_model=False, decoder=False):
        self.model.train()
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                if self.arg.decoder is not None:
                    ...
                    # Dynamic Random Ratio
                    # output, rec, ori = self.model(data, F.one_hot(label, num_classes=self.num_class), epoch=epoch)
            ...
            # TODO weighted loss function
            if self.arg.weighted_loss == 'dynamic':
                # beta = float(format(epoch + 1 / self.arg.num_epoch, '.2f'))
                beta = epoch / self.arg.num_epoch
                w1 = (1 - beta) * 0.1 + beta * 0.9
                w2 = (1 - beta) * 0.9 + beta * 0.1
                total_loss = w1 * loss + w2 * loss_rec
                # total_loss = beta * loss + (1 - beta) * loss_rec
            elif 'weighted' in self.arg.weighted_loss:
                beta = float(self.arg.weighted_loss.split('-')[-1])
                total_loss = beta * loss + (1 - beta) * loss_rec
            else:
            ...
            if self.arg.decoder is not None:
                self.train_writer.add_scalar('loss_rec', loss_rec.data.item(), self.global_step)
            # statistics
        ...
        self.print_log(
            '\tMean training loss: {:.4f}. loss_rec: {:.4f}. Mean training acc: {:.2f}%.'.format(np.mean(loss_value),
                                                                                                 np.mean(loss_value2),
                                                                                                 np.mean(
                                                                                                     acc_value) * 100))
    ...
    def eval(self, epoch, save_score=False, loader_name=['val'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            if '_2' in self.arg.feeder:
                accuracy, video_acc = self.data_loader[ln].dataset.top_k(score, 1, video_level=True)
                video_f1_score = self.data_loader[ln].dataset.f1_score_video(score)
            else:
            ...
            if video_acc > self.best_video_acc:
                self.best_video_acc = video_acc
                self.best_video_acc_epoch = epoch + 1

            print('Accuracy: ', accuracy, ' model: ', self.arg.model_saved_name)
            if self.arg.phase == 'train':
                self.val_writer.add_scalar('loss', loss, self.global_step)
                self.val_writer.add_scalar('loss_rec', loss_rec, self.global_step)
                self.val_writer.add_scalar('acc', accuracy, self.global_step)
                self.val_writer.add_scalar('video_acc', video_acc, self.global_step)
                self.val_writer.add_scalar('video_f1_score', video_f1_score, self.global_step)

            score_dict = dict(
                zip(self.data_loader[ln].dataset.sample_name, score))
            self.print_log('\tMean {} loss of {} batches: loss: {:.4f}. loss_rec: {:.4f}.'.format(
                ln, len(self.data_loader[ln]), loss, loss_rec))
            if '_2' in self.arg.feeder:
            ...
            save_csv_dir = self.arg.work_dir + '/confusion_matrices'
            if not os.path.exists(save_csv_dir):
                os.makedirs(save_csv_dir)
            with open('{}/epoch{}_{}_each_class_acc.csv'.format(save_csv_dir, epoch + 1, ln), 'w') as f:
                writer = csv.writer(f)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            print('The best clip model weights path:', weights_path)
            ...
            self.eval(epoch=self.best_acc_epoch - 1, save_score=True, loader_name=['val'], wrong_file=wf,
                      result_file=rf)
            self.arg.print_log = True

            num_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
            self.print_log(f'Best Clip Accuracy: [Epoch {self.best_acc_epoch}] {self.best_acc}')
            # self.print_log(f'Epoch number: {self.best_acc_epoch}')
            self.print_log(f'Best Video Accuracy: [Epoch {self.best_video_acc_epoch}] {self.best_video_acc}')
            # self.print_log(f'Epoch number: {self.best_video_acc_epoch}')
            self.print_log(f'Model name: {self.arg.work_dir}')
            self.print_log(f'Model total number of params: {num_params}')
            self.print_log(f'Weight decay: {self.arg.weight_decay}')
            self.print_log(f'Base LR: {self.arg.base_lr}')
            self.print_log(f'Batch Size: {self.arg.batch_size}')
            self.print_log(f'Val Batch Size: {self.arg.val_batch_size}')
            ...
            self.eval(epoch=0, save_score=self.arg.save_score, loader_name=['val'], wrong_file=wf, result_file=rf)
            self.print_log('Done.\n')
